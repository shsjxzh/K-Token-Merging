...
def train_worker(
    ...
    compressor = load_compressor(
        ...
        device=device,
        checkpoint_path=compressor_path,
    )
    ...
    for epoch in range(args.epochs_per_stage):
        ...
        progress = tqdm(train_loader, desc=f"{stage} epoch {epoch}", disable=(rank != 0), leave=False)
        for batch_index, (prompts, answers) in enumerate(progress, start=1):
            ...
            if args.save_steps and global_step % args.save_steps == 0 and rank == 0:
                save_artifacts(
                    output_dir=args.output_dir / stage / "step_last",
...
def eval_worker(rank: int, world_size: int, stage: str, args: argparse.Namespace) -> None:
    ...
    compressor = load_compressor(
        ...
        merge_factor=args.merge_factor,
        device=device,
        checkpoint_path=checkpoint_dir / "encoder.pth",
    )
    ...
    for prompts, answers in tqdm(test_loader, desc="eval", disable=(rank != 0), leave=False):
    ...
    if rank == 0:
        ...
        payload = {"stage": stage, "accuracy": accuracy}
        save_json(checkpoint_dir / "eval_accuracy.json", payload)
...
def run_evaluate(args: argparse.Namespace) -> None:
    ...
    mp.spawn(
        eval_worker,
        args=(world_size, args.stage, args),
        nprocs=world_size,
        join=True,
    )

    payload_path = args.checkpoint_dir / "eval_accuracy.json"
